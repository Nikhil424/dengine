...
def load_current_resource
  @current_resource = Chef::Resource::SplunkIdxApp.new(new_resource.name)
...
action :install do
  unless current_resource.version == new_resource.version
    ...
    execute 'install app' do
      ...
      cwd idx_base_path
      creates ::File.join(idx_base_path, new_resource.name)
      user splunk_user
      group splunk_user
...
action :remove do
  unless current_resource.version.nil?
    ...
    dir = ::File.join(idx_base_path, new_resource.name)
...
def current_version
  conf = ::File.join(idx_base_path, new_resource.name, 'default', 'app.conf')
