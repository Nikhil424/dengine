...
module Engine
  class DengineAppUpdate < Chef::Knife

    banner "knife app update (options)"
    ...
    def run
      ...
      data_bag_dir = "/var/lib/jenkins/chef-repo/data_bags/#{name}"
      ...
      if ensure_dir(data_bag_dir) == true
        ...
        system("mkdir /var/lib/jenkins/chef-repo/data_bags/#{name}")
    ...
    def update_data_bag_item(file_location,id,name)
      ...
      if ensure_file(file_location) == true
        ...
        system("cp /var/lib/jenkins/chef-repo/templates/#{name}.json /var/lib/jenkins/chef-repo/data_bags/#{name}/#{name}.json")
        system("knife data bag from file #{name} #{file_location}")
        sleep(10)
        update_data_bag(file_location,id,name)
